...
export default function DeityHub() {
  ...
  const id = Array.isArray(rawParams?.slug) ? rawParams.slug[0] : (rawParams?.slug as string);
      ...
      <section className="py-12 bg-[var(--color-brand-bg)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-end mb-8">
            ...
            <Link href="/mantras" className="text-[var(--color-brand-purple)] font-medium hover:text-[var(--color-brand-gold-dark)] flex items-center gap-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {SHIVA_MANTRAS.map((mantra, idx) => (
              <Link
                key={idx}
                href={`/mantras/${mantra.name.toLowerCase().replace(/\s+/g, '-')}`}
      ...
      <section className="py-12 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-end mb-8">
            ...
            <Link href="/temples" className="text-[var(--color-brand-purple)] font-medium hover:text-[var(--color-brand-gold-dark)] flex items-center gap-1">
              View All <ArrowRight size={16} />
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {SHIVA_TEMPLES.map((temple, idx) => (
              <Link key={idx} href={`/temples/${temple.name.toLowerCase()}`} className={`scroll-from-right scroll-stagger-${idx + 1} interactive-card group rounded-xl overflow-hidden border border-gray-200 shadow-sm bg-white`}>
